...
public:
     ...
     */
    void Set(int idx, /*!< The index of the value in the list that is to be changed. */
     ...
     */
    param_t Get(int idx /*!< The index if the value to retrieve. */ ) const
        { return idx>=GetSize() ? 0 : values[idx]; }

    //! Operator notation for Get().
    param_t operator[](int idx) const
        { return Get(idx); }
